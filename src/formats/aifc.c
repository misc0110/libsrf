...
// wrapper to have in-memory files
typedef struct {
    char *data;
    size_t pos;
    size_t len;
...
static MFILE *mopen_read(char *buffer, int len) {
...
static size_t mfread(void *ptr, size_t size, size_t nmemb, MFILE *stream) {
    size_t i;
    char *data = (char *) ptr;
    for (i = 0; i < size * nmemb; i++) {
        data[i] = stream->data[stream->pos + i];
    }
    stream->pos += size * nmemb;
    return nmemb;
}

static int mfgetc(MFILE *stream) {
    int c = (unsigned char)(stream->data[stream->pos]);
    stream->pos++;
...
static long mftell(MFILE *stream) {
    return stream->pos;
}

static int mfclose(MFILE *stream) {
    if (stream->type == 1) {
        free(stream->data);
    }
    free(stream);
...
static int mfseek(MFILE *stream, long int off, int whence) {
    if(whence == SEEK_SET) {
        stream->pos = off;
    } else if(whence == SEEK_CUR) {
        stream->pos += off;
    } else if(whence == SEEK_END) {
        stream->pos = stream->len - 1 + off;
    } else {
        return -1;
    }
    return 0;
...
static int mferror(MFILE* stream) {
    return 0;
}
...
static void decode_chunk(unsigned char *input, unsigned char *output) {
    char d;
    int i;
    short predictor;
    short step_index;
...
static int get_type(MFILE *f, long *size) {
    int ret;

    unsigned char buf[8];

    ret = mfread(buf, 1, 8, f);
...
static unsigned char *itole(unsigned long l, unsigned char *t) {
    *t = l & 255;
    *(t + 1) = (l >> 8) & 255;
    *(t + 2) = (l >> 16) & 255;
    *(t + 3) = (l >> 24) & 255;
...
static unsigned char *stole(unsigned short l, unsigned char *t) {
    *t = l & 255;
    *(t + 1) = (l >> 8) & 255;
    return t;
}
...
/* only 128 if mono, 256 if stereo */
...
char *libsrf_aifc2wav(char *aifc_input, int len, int *out_len) {
    long size;
    int flags = 0;
    ...
    MFILE *fin = mopen_read(aifc_input, len);
    MFILE *fout = mopen_write();
...
*/
    ...
    /* write header of wav */
    /* start with RIFF */
    if (mfwrite("RIFF", 1, 4, fout) != 4) goto out_error;
    ...
    if (mfwrite(itole(riff_size + 36, buf), 1, 4, fout) != 4) goto out_error;

    /* then WAVEfmt */
    if (mfwrite("WAVEfmt ", 1, 8, fout) != 8) goto out_error;

    /* length of fmt chunk (16) */
    if (mfwrite(itole(16, buf), 4, 1, fout) != 1) goto out_error;

    /* 1 => PCM */
    if (mfwrite(stole(1, buf), 2, 1, fout) != 1) goto out_error;

    /* nb of channels */
    if (mfwrite(stole(nb_chans, buf), 2, 1, fout) != 1) goto out_error;

    /* freq */
    if (mfwrite(itole((int) freq, buf), 4, 1, fout) != 1) goto out_error;

    /* don't know what */
    if (mfwrite(itole((int) freq * nb_chans * (nb_bits / 8), buf), 4, 1, fout) != 1)
        goto out_error;

    /* don't know what */
    if (mfwrite(stole(nb_chans * (nb_bits / 8), buf), 2, 1, fout) != 1)
        goto out_error;

    /* nb bits */
    if (mfwrite(stole(nb_bits, buf), 2, 1, fout) != 1) goto out_error;

    /* data chunk */
    if (mfwrite("data", 4, 1, fout) != 1) goto out_error;

    /* data size */
    if (mfwrite(itole(riff_size, buf), 1, 4, fout) != 4) goto out_error;

    /* let's kick all the frames out */
    if (mfseek(fin, ssnd_pos, SEEK_SET) == -1) goto out_error;
    ...
    while (nb_frames) {
        ...
        if (mfread(in, 1, 2, fin) != 2) {
            goto bad_file;
        }
        if (mfwrite(in, 1, 2, fout) != 2)
            goto out_error;
    ...
    while (nb_frames) {
        ...
        if (mfwrite(out, 1, nb_chans == 2 ? 256 : 128, fout) !=
            (nb_chans == 2 ? 256 : 128))
            goto out_error;
    ...
    file_done:
    wav = (char*)malloc(fout->len);
    memcpy(wav, fout->data, fout->len);
    *out_len = fout->len;
    mfclose(fin);
    mfclose(fout);
    return wav;
